...
- **The Ladder** — a fixed 10-question climb that ramps T1 → T7 within the run. Pick your confidence (1×–5×) from the inline stake row before
...
```text
node verify/game-core.test.mjs   # engine unit tests (87 checks, canonical bank)
node verify/hero.test.mjs        # Choose Your Hero engine + data tests (32 checks, KJV-verified)
node verify/check.mjs            # content vs KJV lockbox — all 6 gates green
