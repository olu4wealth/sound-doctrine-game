...
// `timeForTier` ignored its argument after the flat-30s change; callers reading it
// as tier-scaled were being misled. Kept as a thin alias for compatibility.
export function timeForTier() {
  return QUESTION_TIME;
}

// Reading floor. The flat 30s is right for most of the bank, but 29 questions
// need more than 20s just to READ — the longest T7 item is 56 words (~22s), which
// left 8s to actually think. This only lifts the clock for those; every question
// that fits comfortably in 30s still gets exactly 30s, so master's pacing stands.
export const READ_WORDS_PER_SEC = 2.6; // KJV English at a comprehension pace
export const MIN_THINK_SECONDS = 12;   // thinking time guaranteed on top of reading
// Cap the lift. A handful of items (four 27-word verse variants to compare) would
// otherwise ask for ~57s on one screen; those are compared, not read linearly.
export const MAX_QUESTION_TIME = 45;
...
export function timeForQuestion(q) {
  const budget = Math.ceil(readingSeconds(q) + MIN_THINK_SECONDS);
  return Math.min(MAX_QUESTION_TIME, Math.max(QUESTION_TIME, budget));
}
...
// step 4 reaches T7 at question 25 — fine for the old endless climb, but a fixed
// 10-question run would top out at T3 and make T4–T7 unreachable. LADDER_TIER_STEP
// is derived from the run length so the last question always lands on T7.
export const LADDER_TIER_STEP = (LADDER_LENGTH - 1) / 6; // = 1.5 for a 10-question climb
